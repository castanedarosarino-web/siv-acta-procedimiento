...
def nuevo_acto_aprehension():
    return {
        "lugar_exacto": "",
        "hora_exacta": "",
        "motivo_aprehension": "",
        "personal_que_procede": "",
        "movil_interviniente": "",
        "dependencia_actuante": "",
        "circunstancia_breve": "",
        "fiscal_intervencion": "",
        "destino_aprehendido": ""
    }
...
def nuevo_arrestado():
    return {
        ...
        "deposito": "NO",
        "detalle_dep": "",
        "usar_datos_acto_propios": False,
        "acto_aprehension_propio": nuevo_acto_aprehension(),
...
def asegurar_arrestado(a):
    ...
    if not isinstance(base.get("acto_aprehension_propio"), dict):
        base["acto_aprehension_propio"] = nuevo_acto_aprehension()
    else:
        acto = nuevo_acto_aprehension()
        acto.update(base.get("acto_aprehension_propio", {}))
        base["acto_aprehension_propio"] = acto
    return base


def asegurar_acto_aprehension(acto):
    base = nuevo_acto_aprehension()
...
def limpiar_widget_keys_bloque_2():
    prefijos = [
        ...
        "usar_testigo_propio", "acto_general", "acto_propio", "usar_datos_acto_propios"
    ]
    borrar = []
...
def obtener_acto_para_arrestado(arrestado):
    if arrestado.get("usar_datos_acto_propios"):
        return asegurar_acto_aprehension(arrestado.get("acto_aprehension_propio", {})), "PROPIO"
    return asegurar_acto_aprehension(st.session_state.get("acto_aprehension_general", nuevo_acto_aprehension())), "GENERAL"
...
def construir_json_sivap():
    return {
        ...
        "acta_aprehension": {
            ...
            "datos_generales_acto": st.session_state.get("acto_aprehension_general", nuevo_acto_aprehension()),
...
def generar_pdf_bloque_2(data):
    ...
    for idx, a in enumerate(arrestados, start=1):
        ...
        acto, tipo_acto = obtener_acto_para_arrestado(a)
        ...
        titulo(f"DATOS PROPIOS DEL ACTO DE APREHENSION ({tipo_acto})")
        mc(f"Lugar exacto de aprehensión: {acto.get('lugar_exacto', '')}")
        mc(f"Hora exacta de aprehensión: {acto.get('hora_exacta', '')}")
        mc(f"Motivo de la aprehensión: {acto.get('motivo_aprehension', '')}")
        mc(f"Personal que procede: {acto.get('personal_que_procede', '')}")
        mc(f"Móvil interviniente: {acto.get('movil_interviniente', '')}")
        mc(f"Dependencia actuante: {acto.get('dependencia_actuante', '')}")
        mc(f"Circunstancia breve del acto: {acto.get('circunstancia_breve', '')}")
        mc(f"Fiscal / intervención comunicada: {acto.get('fiscal_intervencion', '')}")
        mc(f"Destino del aprehendido: {acto.get('destino_aprehendido', '')}")
        pdf.ln(2)
...
if "acto_aprehension_general" not in st.session_state:
    st.session_state.acto_aprehension_general = nuevo_acto_aprehension()
    ...
    if archivo_json is not None:
        try:
            ...
            if st.button("✅ Incorporar datos al BLOQUE 2", key="btn_incorporar_json_bloque_2"):
                ...
                datos_generales_acto = acta_aprehension.get("datos_generales_acto") or acta_aprehension.get("datos_generales") or {}
                st.session_state.acto_aprehension_general = asegurar_acto_aprehension(datos_generales_acto)
...
# ============================================================
# DATOS GENERALES DEL ACTO DE APREHENSION
# ============================================================
st.divider()
st.subheader("📝 Datos generales del acto de aprehensión")
st.caption("Se usan por defecto en cada acta individual. Si un aprehendido tuvo otro lugar, hora o circunstancia, activá datos propios dentro de su sección.")

acto_general = asegurar_acto_aprehension(st.session_state.acto_aprehension_general)
c1, c2 = st.columns(2)
acto_general["lugar_exacto"] = c1.text_input("Lugar exacto de aprehensión", value=acto_general.get("lugar_exacto", ""), key="acto_general_lugar")
acto_general["hora_exacta"] = c2.text_input("Hora exacta de aprehensión", value=acto_general.get("hora_exacta", ""), key="acto_general_hora")
acto_general["motivo_aprehension"] = st.text_input("Motivo de la aprehensión", value=acto_general.get("motivo_aprehension", ""), key="acto_general_motivo")
c3, c4, c5 = st.columns(3)
acto_general["personal_que_procede"] = c3.text_input("Personal que procede", value=acto_general.get("personal_que_procede", ""), key="acto_general_personal")
acto_general["movil_interviniente"] = c4.text_input("Móvil interviniente", value=acto_general.get("movil_interviniente", ""), key="acto_general_movil")
acto_general["dependencia_actuante"] = c5.text_input("Dependencia actuante", value=acto_general.get("dependencia_actuante", ""), key="acto_general_dependencia")
acto_general["circunstancia_breve"] = st.text_area("Circunstancia breve del acto", value=acto_general.get("circunstancia_breve", ""), key="acto_general_circunstancia")
c6, c7 = st.columns(2)
acto_general["fiscal_intervencion"] = c6.text_input("Fiscal / intervención comunicada", value=acto_general.get("fiscal_intervencion", ""), key="acto_general_fiscal")
acto_general["destino_aprehendido"] = c7.text_input("Destino del aprehendido", value=acto_general.get("destino_aprehendido", ""), key="acto_general_destino")
st.session_state.acto_aprehension_general = acto_general
...
for i in range(st.session_state.cantidad):
    ...
    with st.expander(f"ARRESTADO / APREHENDIDO {i + 1}", expanded=True):
        ...
        st.subheader("📝 Datos propios del acto de aprehensión")
        a["usar_datos_acto_propios"] = st.checkbox(
            "Este aprehendido tiene lugar/hora/circunstancia propia",
            value=bool(a.get("usar_datos_acto_propios")),
            key=f"usar_datos_acto_propios{i}"
        )
        if a["usar_datos_acto_propios"]:
            acto_p = asegurar_acto_aprehension(a.get("acto_aprehension_propio", {}))
            c30, c31 = st.columns(2)
            acto_p["lugar_exacto"] = c30.text_input("Lugar exacto de aprehensión", value=acto_p.get("lugar_exacto", ""), key=f"acto_propio_lugar{i}")
            acto_p["hora_exacta"] = c31.text_input("Hora exacta de aprehensión", value=acto_p.get("hora_exacta", ""), key=f"acto_propio_hora{i}")
            acto_p["motivo_aprehension"] = st.text_input("Motivo de la aprehensión", value=acto_p.get("motivo_aprehension", ""), key=f"acto_propio_motivo{i}")
            c32, c33, c34 = st.columns(3)
            acto_p["personal_que_procede"] = c32.text_input("Personal que procede", value=acto_p.get("personal_que_procede", ""), key=f"acto_propio_personal{i}")
            acto_p["movil_interviniente"] = c33.text_input("Móvil interviniente", value=acto_p.get("movil_interviniente", ""), key=f"acto_propio_movil{i}")
            acto_p["dependencia_actuante"] = c34.text_input("Dependencia actuante", value=acto_p.get("dependencia_actuante", ""), key=f"acto_propio_dependencia{i}")
            acto_p["circunstancia_breve"] = st.text_area("Circunstancia breve del acto", value=acto_p.get("circunstancia_breve", ""), key=f"acto_propio_circunstancia{i}")
            c35, c36 = st.columns(2)
            acto_p["fiscal_intervencion"] = c35.text_input("Fiscal / intervención comunicada", value=acto_p.get("fiscal_intervencion", ""), key=f"acto_propio_fiscal{i}")
            acto_p["destino_aprehendido"] = c36.text_input("Destino del aprehendido", value=acto_p.get("destino_aprehendido", ""), key=f"acto_propio_destino{i}")
            a["acto_aprehension_propio"] = acto_p
        else:
            a["acto_aprehension_propio"] = asegurar_acto_aprehension(a.get("acto_aprehension_propio", {}))
