...
with tab_act:
    ...
    with c1:
        st.session_state.b8_actuacion["numero_acta"] = st.text_input("Nro. de acta / procedimiento", value=st.session_state.b8_actuacion.get("numero_acta", ""), key="b8_numero_acta")
        st.session_state.b8_actuacion["fecha"] = str(st.date_input("Fecha", value=date.fromisoformat(st.session_state.b8_actuacion.get("fecha", str(date.today()))) if re.match(r"\d{4}-\d{2}-\d{2}", st.session_state.b8_actuacion.get("fecha", "")) else date.today(), key="b8_fecha"))
        st.session_state.b8_actuacion["hora_inicio"] = st.text_input("Hora inicio / intervención", value=st.session_state.b8_actuacion.get("hora_inicio", ""), key="b8_hora_inicio")
        st.session_state.b8_actuacion["hora_cierre"] = st.text_input("Hora cierre", value=st.session_state.b8_actuacion.get("hora_cierre", ""), key="b8_hora_cierre")
    with c2:
        st.session_state.b8_actuacion["unidad_regional"] = st.text_input("Unidad Regional", value=st.session_state.b8_actuacion.get("unidad_regional", "UNIDAD REGIONAL II"), key="b8_unidad")
        st.session_state.b8_actuacion["dependencia"] = st.text_input("Dependencia", value=st.session_state.b8_actuacion.get("dependencia", ""), key="b8_dependencia")
        st.session_state.b8_actuacion["localidad"] = st.text_input("Localidad", value=st.session_state.b8_actuacion.get("localidad", "Rosario"), key="b8_localidad")
        st.session_state.b8_actuacion["movil"] = st.text_input("Móvil", value=st.session_state.b8_actuacion.get("movil", ""), key="b8_movil")
    with c3:
        st.session_state.b8_actuacion["incidencia"] = st.text_input("Incidencia / comisión 911", value=st.session_state.b8_actuacion.get("incidencia", ""), key="b8_incidencia")
        st.session_state.b8_actuacion["lugar_hecho"] = st.text_area("Lugar del hecho", value=st.session_state.b8_actuacion.get("lugar_hecho", ""), height=80, key="b8_lugar_hecho")
        st.session_state.b8_actuacion["lugar_aprehension"] = st.text_area("Lugar de aprehensión", value=st.session_state.b8_actuacion.get("lugar_aprehension", ""), height=80, key="b8_lugar_aprehension")

    st.session_state.b8_actuacion["personal_actuante"] = st.text_area("Personal actante / interviniente", value=st.session_state.b8_actuacion.get("personal_actuante", ""), height=100, key="b8_personal")

    colf1, colf2 = st.columns(2)
    with colf1:
        st.session_state.b8_actuacion["mesa_enlace_fiscalia"] = st.text_input("Mesa de Enlace / Fiscalía", value=st.session_state.b8_actuacion.get("mesa_enlace_fiscalia", ""), key="b8_fiscalia")
    with colf2:
        st.session_state.b8_actuacion["disposicion_fiscal"] = st.text_input("Disposición fiscal / indicación recibida", value=st.session_state.b8_actuacion.get("disposicion_fiscal", ""), key="b8_disposicion")
...
with tab_acta:
    ...
    relato_principal = st.text_area(
        ...
        value=st.session_state.get("b8_relato_principal_manual", relato_sugerido),
        height=240,
    ...
    st.session_state.b8_acta_texto = st.text_area(
        "Acta final editable",
        value=st.session_state.b8_acta_texto,
        height=520,
