...
def importar_json_bloque3(data):
    ...
    if firma:
        ...
        subida_b64 = firma.get("firma_subida_base64") or ""
        if firma_base64_valida(canvas_b64):
            setv("b3_firma_canvas_b64", canvas_b64)
            setv("b3_modo_firma", "Firma digital en pantalla")
        if firma_base64_valida(subida_b64):
            setv("b3_firma_subida_b64", subida_b64)
            setv("b3_firma_subida_nombre", firma.get("firma_subida_nombre") or "firma_importada.png")
...
def obtener_firma_base64():
    # Prioridad: firma dibujada; respaldo: firma subida.
    return val("b3_firma_canvas_b64") or val("b3_firma_subida_b64") or ""
...
def armar_json_exportacion():
    ...
    datos_bloque = {
        ...
        "firma": {
            ...
            "firma_subida_base64": val("b3_firma_subida_b64"),
        }
    }
...
with tabs[5]:
    ...
    st.caption("Firme dentro del recuadro blanco. La firma digital se guarda en el JSON y se inserta en el PDF.")
    ...
    if CANVAS_OK:
        ...
        st.warning("Canvas no disponible en este dispositivo. Suba una foto de la firma como respaldo.")

    firma_file = st.file_uploader("Respaldo técnico: subir foto de firma si el canvas no funciona", type=["jpg", "jpeg", "png"], key="b3_upload_firma")
    if firma_file:
