...
def construir_relato_final(guardar=False):
    """Construye el relato final.
    Importante: solo escribe en st.session_state si guardar=True.
    Esto evita errores de Streamlit cuando el widget b3_relato_final ya fue creado.
    """
    partes = []
...
def armar_json_exportacion():
    ...
    return {
        ...
        "datos_entrevista": datos_bloque,  # compatibilidad con BLOQUE 3 anteriores
        ...
        try:
            ...
            if not es_json_bloque3(data):
                ...
                if st.button("📥 Incorporar datos al BLOQUE 3 del actante", use_container_width=True):
                    ...
                    st.success("Datos incorporados. Revise las pestañas siguientes y corrija lo necesario. Si algún campo venía vacío en el JSON, quedará vacío para completar manualmente.")
                    if GUARDADO_OK:
                        try:
