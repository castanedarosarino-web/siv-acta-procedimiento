...
# -----------------------------------------------------
# ESTADO SEGURO DEL BLOQUE 3
# -----------------------------------------------------
# Streamlit elimina del session_state los widgets que dejan de renderizarse
# cuando se navega por secciones con st.radio. Por eso BLOQUE 3 mantiene
# una copia interna permanente (b3_data_segura) separada de los widgets.
# Los widgets pueden aparecer/desaparecer; la informacion real queda aca.

DATA_KEYS_B3 = list(DEFAULTS.keys())

if "b3_data_segura" not in st.session_state or not isinstance(st.session_state.get("b3_data_segura"), dict):
    st.session_state["b3_data_segura"] = {}

for k, v in DEFAULTS.items():
    if k not in st.session_state["b3_data_segura"]:
        st.session_state["b3_data_segura"][k] = v
    if k not in st.session_state:
        st.session_state[k] = st.session_state["b3_data_segura"].get(k, v)

# =====================================================
# UTILIDADES
# =====================================================

def b3_persistir_estado_actual():
    """Copia a b3_data_segura solo datos reales del BLOQUE 3.
    No guarda botones, uploaders, downloads ni claves internas de Streamlit.
    """
    data = st.session_state.setdefault("b3_data_segura", {})
    for k in DATA_KEYS_B3:
...
def val(k, default=""):
    ...
    return st.session_state.get("b3_data_segura", {}).get(k, default)


def setv(k, v):
    st.session_state[k] = v
    if k in DATA_KEYS_B3:
        st.session_state.setdefault("b3_data_segura", {})[k] = v
...
# Persistencia interna antes del autoguardado global.
# Esto evita perder datos al navegar entre secciones con st.radio.
try:
    b3_persistir_estado_actual()
except Exception:
    pass
