...
with st.expander("📥 0. Importar JSON recibido por WhatsApp", expanded=False):
    ...
    st.info("Use este sector cuando un colaborador envía por WhatsApp un JSON de BLOQUE 3. El formulario se completa y el actante puede revisar/corregir antes de imprimir o exportar.")
    ...
    if archivo is not None:
        try:
            ...
            if not es_json_bloque3(data):
                ...
                st.markdown("#### Vista previa técnica del JSON")
                st.json(data)
...
with st.expander("📤 6. PDF / JSON", expanded=False):
    ...
    st.markdown("#### Vista técnica JSON")
