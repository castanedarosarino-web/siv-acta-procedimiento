...
def generar_pdf_completo_b3():
    ...
    pdf.cell(0, 8, limpiar_pdf("ACTA DE ENTREVISTA A VICTIMA / DAMNIFICADO"), ln=True, align="C")
    pdf.ln(2)
    ...
    label_value_line("Lugar:", lugar)
    label_value_line("Dependencia:", dep)
    label_value_line("Personal que recepciona:", personal)
    datos_func = ""
    if jerarquia:
        datos_func += f"Jerarquia: {jerarquia}"
    if ni:
        datos_func += (" - " if datos_func else "") + f"NI: {ni}"
    if datos_func:
        label_value_line("Datos del personal:", datos_func)
