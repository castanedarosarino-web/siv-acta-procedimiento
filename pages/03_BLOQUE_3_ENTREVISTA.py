...
def generar_pdf_completo_b3():
    ...
    if dni:
        texto += f", DNI Nro. {dni}"
    texto += ", respecto de los derechos que le asisten en su caracter de victima/damnificado/a en las presentes actuaciones, conforme normativa vigente."
    pdf_multi(pdf, texto)
    pdf.ln(4)
    ...
    derechos = [
        "A recibir trato digno, respetuoso y no discriminatorio por parte de las autoridades intervinientes.",
        "A que se resguarde su intimidad, dignidad y seguridad durante la intervencion policial y las actuaciones posteriores.",
        "A que se reduzcan al minimo las molestias derivadas del procedimiento, evitando intervenciones innecesariamente reiteradas o revictimizantes.",
        "A recibir informacion clara sobre los derechos que le asisten desde la primera intervencion.",
        "A ser informada, cuando corresponda, sobre el estado del procedimiento, la intervencion fiscal y las medidas dispuestas.",
        "A aportar informacion, documentacion, testigos, imagenes, videos, capturas, objetos u otros elementos utiles para la investigacion.",
        "A solicitar medidas de proteccion para su persona, grupo familiar, testigos u otras personas vinculadas, cuando existieren circunstancias que lo justifiquen.",
        "A recibir asistencia, orientacion, contencion y derivacion a organismos especializados de atencion a victimas.",
        "A designar domicilio, telefono, correo electronico u otro medio idoneo para recibir comunicaciones o notificaciones vinculadas al procedimiento.",
        "A solicitar la restitucion de efectos propios recuperados o secuestrados, cuando correspondiere y conforme disposicion de autoridad competente.",
        "A requerir asesoramiento, patrocinio o asistencia juridica en los casos y por las vias que correspondan.",
        "A ser escuchada por la autoridad competente antes de decisiones que puedan afectar sus intereses, en los supuestos legalmente previstos.",
        "A conocer, cuando corresponda y por los canales legales pertinentes, la situacion procesal de la persona imputada o aprehendida.",
        "A ampliar denuncia, aportar nuevos datos o solicitar que se reciban medidas utiles para el esclarecimiento del hecho.",
        "A ser informada sobre vias institucionales disponibles para efectuar consultas, requerir asistencia o aportar informacion complementaria."
    ]
    for idx, d in enumerate(derechos, start=1):
        if pdf.get_y() > 245:
            pdf.add_page()
            section_title("DERECHOS INFORMADOS - CONTINUACION")
            pdf.set_font("Arial", "", 8.6)
        pdf_multi(pdf, f"{idx}. {d}", alto=4.7)
    pdf.ln(4)

    section_title("CONSTANCIA")
    pdf.set_font("Arial", "", 9)
    constancia = (
        "La persona notificada manifiesta quedar debidamente informada de los derechos precedentemente consignados, "
        "firmando al pie junto al personal policial interviniente para constancia."
    )
    pdf_multi(pdf, constancia, alto=5)
    pdf.ln(4)
