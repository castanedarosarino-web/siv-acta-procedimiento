...
# -----------------------------------------------------
# Limpieza de claves transitorias de Streamlit
# -----------------------------------------------------
# El sistema de guardado no debe restaurar valores de botones,
# download_button ni file_uploader. Streamlit no permite que esas
# claves sean seteadas desde st.session_state.
def b3_es_clave_transitoria(k):
    prefijos = (
        "b3_guardar_avance_",
        "b3_volver_",
        "b3_siguiente_",
        "b3_down_",
        "b3_import_json",
        "b3_import_json_pendiente_archivo",
    )
    return str(k).startswith(prefijos)

def b3_limpiar_claves_transitorias():
    for _k in list(st.session_state.keys()):
        if b3_es_clave_transitoria(_k):
            try:
                del st.session_state[_k]
            except Exception:
                pass

# Si el usuario recupera un borrador anterior que guardó botones/file_uploader,
# limpiamos antes de crear widgets para evitar StreamlitAPIException.
b3_limpiar_claves_transitorias()
...
# -----------------------------------------------------
# Navegación segura por secciones
# -----------------------------------------------------
# Se usa st.radio en lugar de st.tabs para que el canvas de firma se renderice bien.
# Los botones de avance quedan al final de cada sección y refuerzan el guardado manual.

def b3_indice_seccion_actual():
    actual = st.session_state.get("b3_seccion_actual", SECCIONES_B3[0])
    if actual not in SECCIONES_B3:
        return 0
    return SECCIONES_B3.index(actual)


def b3_ir_a_seccion(nombre):
    if nombre in SECCIONES_B3:
        st.session_state["b3_seccion_actual"] = nombre


def b3_guardar_avance_manual():
    # Antes de guardar, eliminamos claves transitorias.
    # Esas claves no son datos policiales y rompen la recuperación.
    b3_limpiar_claves_transitorias()
    if GUARDADO_OK:
        try:
            autoguardar_bloque(BLOQUE_ID)
        except Exception as e:
            st.session_state["_b3_guardado_manual_error"] = str(e)
            return
    st.session_state["_b3_ultimo_guardado_manual"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    st.session_state["_b3_guardado_manual_ok"] = True


def b3_barra_navegacion():
    st.divider()
    st.caption("Control de avance del BLOQUE 3")
    idx = b3_indice_seccion_actual()
    ...
    with cols[0]:
        st.button(
            ...
            on_click=b3_guardar_avance_manual,
        )

    with cols[1]:
        anterior = SECCIONES_B3[idx - 1] if idx > 0 else SECCIONES_B3[0]
        st.button(
            "◀ Volver",
            use_container_width=True,
            disabled=(idx == 0),
            on_click=b3_ir_a_seccion,
            args=(anterior,),
        )

    with cols[2]:
        siguiente = SECCIONES_B3[idx + 1] if idx < len(SECCIONES_B3) - 1 else SECCIONES_B3[-1]
        st.button(
            "Siguiente ▶",
            use_container_width=True,
            disabled=(idx == len(SECCIONES_B3) - 1),
            on_click=b3_ir_a_seccion,
            args=(siguiente,),
        )

    with cols[3]:
        ultimo = st.session_state.get("_b3_ultimo_guardado_manual", "")
        if ultimo:
            st.success(f"Último guardado: {ultimo.split()[-1]} hs")
        else:
            st.info("Sin guardado manual en esta sesión")

    if st.session_state.pop("_b3_guardado_manual_ok", False):
        st.success("✅ Avance del BLOQUE 3 guardado correctamente.")
    if st.session_state.get("_b3_guardado_manual_error"):
        st.warning("No se pudo confirmar el guardado S.I.V.A.P.: " + str(st.session_state.pop("_b3_guardado_manual_error")))


seccion = st.radio(
    "Secciones del BLOQUE 3",
    SECCIONES_B3,
    horizontal=True,
    key="b3_seccion_actual",
    label_visibility="collapsed"
)

# 0 Importar
if seccion == "0. Importar JSON":
    ...
    archivo = st.file_uploader("Subir JSON BLOQUE 3 recibido por WhatsApp", type=["json"])
...
# 3 Analisis dinamico
if seccion == "3. Análisis S.I.V.A.P.":
...
# 4 Relato final
if seccion == "4. Relato final / Noticia":
...
# 5 FIRMA DIGITAL
if seccion == "5. FIRMA DIGITAL":
    st.subheader("🖊️ FIRMA DIGITAL DE LA PERSONA ENTREVISTADA")
    st.caption("Firme dentro del recuadro blanco. Esta firma se incorporará al PDF y viajará en el JSON para WhatsApp.")
    if CANVAS_OK:
...
# 6 PDF / JSON
if seccion == "6. PDF / JSON":
    st.subheader("PDF completo y JSON")
    st.info("El PDF completo genera Acta de Entrevista y Notificación de Derechos de la Víctima. La firma de la pestaña FIRMA DIGITAL se inserta en el PDF y viaja en el JSON para WhatsApp.")

    if firma_base64_valida(obtener_firma_base64()):
        st.success("Firma digital cargada. Será incorporada al PDF y al JSON.")
    else:
        st.warning("Todavía no se detectó firma digital. Primero firme en la pestaña FIRMA DIGITAL.")
...
# Barra de navegación/guardado al final de cada sección activa.
b3_barra_navegacion()
