...
def etiquetas_delitos_detectados(d, tipos):
    etiquetas = []
    if "robo_arma_violencia_lesiones" in tipos:
        etiquetas.append("Robo con arma/violencia y posibles lesiones")
    elif d.get("robo"):
        etiquetas.append("Robo / arrebato / sustracción con violencia o intimidación")
    if d.get("hurto"):
        etiquetas.append("Hurto")
    if d.get("amenazas"):
    ...
    if d.get("sexual"):
        etiquetas.append("Delito contra la integridad sexual")
    if d.get("privada"):
...
def relato_mas_delitos_manual(relato):
    manual = st.session_state.get("b3_delitos_manual", [])
    if not isinstance(manual, list):
        manual = []
    return str(relato or "") + " " + " ".join(manual)
...
def construir_preguntas_guia(relato):
    relato_extendido = relato_mas_delitos_manual(relato)
    d, tipos = analizar_relato_interno(relato_extendido)
    preguntas = []
    ...
    if d["robo"] or d["amenazas"] or d["lesiones"] or menciona_autor:
        preguntas.extend([
            pregunta("autor_descripcion", "¿Cómo era la persona que cometió el hecho?",
                     "Edad aproximada, altura, contextura, tez, cabello, vestimenta, señas particulares, tatuajes, gorra, mochila u otros datos."),
            pregunta("autor_observacion", "¿La víctima lo vio bien, lo conoce o podría reconocerlo?",
                     "Precisar si lo conoce, si es la primera vez que lo ve y si podría reconocerlo nuevamente."),
            pregunta("autor_conexion_aprehendido", "Si hubo persona retenida/aprehendida, ¿es la misma que robó, amenazó o lesionó?",
                     "Esta respuesta es clave para justificar que el arrestado es el mismo sujeto señalado por la víctima.")
        ])
...
def construir_relato_final(relato_crudo, respuestas):
    ...
    orden = [
        "base_modo_tiempo_lugar", "autor_descripcion", "autor_observacion", "autor_conexion_aprehendido",
        "amenaza_frase", "amenaza_amedrentamiento",
    ...
    for oid in orden:
        ...
        if not r:
            continue
        if oid == "autor_descripcion":
            partes.append(f"Puedo describir al autor como {r}.")
        elif oid == "autor_observacion":
            partes.append(f"Respecto de la observación del autor, manifiesto que {r}.")
        elif oid == "autor_conexion_aprehendido":
            partes.append(f"Asimismo, hago saber que {r}.")
        elif oid == "amenaza_frase":
...
def generar_noticia_criminis(filiacion, relato_final, respuestas):
    """
    Noticia criminis para incorporar al BLOQUE 1 / BLOQUE 8.
    Estilo policial:
    Seguidamente se entrevista a quien dijo llamarse...
    quien manifestó que...
    Breve y suficiente para justificar intervención o aprehensión.
    No reemplaza el acta de entrevista.
    """
    ...
    relato = str(relato_final or "")
    d, tipos = analizar_relato_interno(relato)

    texto = f"Seguidamente se entrevista a quien dijo llamarse {nombre}"
    if dni:
        texto += f", DNI Nro. {dni}"
    if domicilio:
        texto += f", domiciliado/a en {domicilio}"
    texto += ", quien manifestó que momentos antes"

    lugar_tiempo = respuestas.get("base_modo_tiempo_lugar", "").strip()
    if lugar_tiempo:
        texto += f", {lugar_tiempo}"

    if d.get("robo") and d.get("arma"):
        texto += ", fue abordada por un sujeto armado o portando un elemento de peligrosidad"
    elif d.get("robo"):
        texto += ", fue abordada por un sujeto"
    elif d.get("amenazas"):
        texto += ", fue amenazada por una persona"
    elif d.get("lesiones"):
        texto += ", fue agredida físicamente por una persona"
    elif d.get("danio"):
        texto += ", tomó conocimiento de daños sufridos"
    elif d.get("hurto"):
        texto += ", advirtió la sustracción de pertenencias"
    else:
        texto += ", aportó datos de interés vinculados al hecho investigado"

    desc_autor = respuestas.get("autor_descripcion", "").strip()
    if desc_autor:
        texto += f", a quien describió como {desc_autor}"

    arma_desc = respuestas.get("arma_descripcion", "").strip()
    if arma_desc:
        texto += f", manifestando que el mismo portaba {arma_desc}"

    amenaza = respuestas.get("amenaza_frase", "").strip()
    if amenaza:
        texto += f", quien bajo amenazas le expresó textualmente “{amenaza}”"

    objeto = respuestas.get("robo_objeto", "").strip()
    if objeto:
        texto += f", intentando y/o logrando sustraerle {objeto}"
    elif d.get("robo"):
        texto += ", intentando y/o logrando sustraerle pertenencias"

    modo = respuestas.get("robo_modo", "").strip()
    if modo:
        texto += f". Asimismo, manifestó que {modo}"
    elif d.get("lesiones") and d.get("robo"):
        texto += ". Asimismo, manifestó que fue golpeada durante el hecho"

    lesion = respuestas.get("lesion_zona", "").strip()
    if lesion:
        texto += f", resultando con {lesion}"

    asistencia = respuestas.get("lesion_asistencia", "").strip()
    if asistencia:
        texto += f", refiriendo además que {asistencia}"

    fuga = respuestas.get("robo_autor_fuga", "").strip()
    if fuga:
        texto += f". En relación a la fuga o posterior retención del autor, manifestó que {fuga}"

    conexion = respuestas.get("autor_conexion_aprehendido", "").strip()
    if conexion:
        texto += f". La entrevistada manifestó que {conexion}"

    observacion = respuestas.get("autor_observacion", "").strip()
    if observacion:
        texto += f". Respecto de la observación o reconocimiento del autor, manifestó que {observacion}"

    reconocimiento = respuestas.get("robo_reconocimiento", "").strip()
    if reconocimiento:
        texto += f". Asimismo, respecto de la posibilidad de reconocerlo, manifestó que {reconocimiento}"

    prueba = respuestas.get("prueba_detalle", "").strip() or respuestas.get("amenaza_medio_prueba", "").strip()
    if prueba:
        texto += f". También hizo saber que {prueba}"

    if respuestas.get("lesion_instancia") == "SÍ":
        texto += ". Asimismo, manifestó que es su deseo instar la acción penal por las lesiones sufridas"
    elif respuestas.get("lesion_instancia") == "NO":
        texto += ". Asimismo, manifestó que no es su deseo instar la acción penal por las lesiones sufridas"
...
def firma_upload_a_path(uploaded_file):
    """
    Permite usar una foto/imagen de firma como alternativa cuando el canvas
    no se muestra correctamente en algún celular.
    """
...
with tabs[2]:
    ...
    d_detectado, tipos_detectados = analizar_relato_interno(relato)
    etiquetas = etiquetas_delitos_detectados(d_detectado, tipos_detectados)
    ...
    if not relato.strip():
        ...
        if etiquetas:
            ...
            st.warning("S.I.V.A.P. no detectó con claridad el delito o situación principal. Revise el relato o agregue una opción manual.")

        st.markdown("### ¿Cree que hay algún otro delito o situación que el sistema no detectó?")
        st.multiselect(
            "Agregar manualmente para activar preguntas clave",
            [
                "robo",
                "robo con arma",
                "amenazas",
        ...
        if "otro" in st.session_state.get("b3_delitos_manual", []):
...
with tabs[3]:
    ...
    st.caption("Sirve para incorporar al BLOQUE 1/BLOQUE 8. Debe ser breve, policial y suficiente para justificar intervención o aprehensión. No reemplaza el relato completo.")
...
with tabs[4]:
    ...
    firma_upload = st.file_uploader(
        "Si no puede firmar en pantalla, suba una foto/imagen de la firma",
        type=["jpg", "jpeg", "png"],
        key="b3_file_uploader_firma"
    )
    ...
    datos_exportar = {
        ...
        "delitos_agregados_por_policia": st.session_state.get("b3_delitos_manual", []),
        "delito_manual_otro": st.session_state.get("b3_delito_manual_otro", ""),
