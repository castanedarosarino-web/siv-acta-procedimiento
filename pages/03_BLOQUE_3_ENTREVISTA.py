...
# =====================================================
# SINCRONIZACION CON APP PRINCIPAL Y RESPALDO EN SESION
# =====================================================
# El BLOQUE 3 no debe volver a pedir datos que ya estan cargados en APP.
# Lee identidad policial y actuacion activa desde session_state global cuando existan.
# Ademas mantiene una copia liviana de los datos reales del bloque para evitar
# perdidas por rerun/descarga/recarga dentro de la misma sesion.

B3_DATA_KEYS = [
    k for k in DEFAULTS.keys()
    if k not in {"b3_modo_firma"}
]
B3_DATA_KEYS += ["b3_modo_firma", "b3_firma_canvas_b64"]
...
def _buscar_en_dicts_globales(posibles):
    """Busca datos dentro de diccionarios globales de APP sin depender de un nombre unico."""
    for _, v in list(st.session_state.items()):
        if not isinstance(v, dict):
            continue
        for key in posibles:
            if key in v and _valor_util(v.get(key)):
                return v.get(key)
    return None


def _buscar_global(posibles, tipo="texto"):
    """Busca primero claves exactas/globales y luego coincidencias por nombre de key."""
    # 1) claves exactas frecuentes
    for key in posibles:
        if key in st.session_state and _valor_util(st.session_state.get(key)):
            return st.session_state.get(key)
    # 2) dentro de dicts globales
    v = _buscar_en_dicts_globales(posibles)
    if _valor_util(v):
        return v
    # 3) busqueda flexible por nombre de key, evitando claves del propio BLOQUE 3
    posibles_low = [p.lower() for p in posibles]
    for k, v in list(st.session_state.items()):
        kl = str(k).lower()
        if kl.startswith("b3_") or kl.startswith("_"):
            continue
        if not _valor_util(v) or isinstance(v, (dict, list, tuple, set)):
            continue
        if any(p in kl for p in posibles_low):
            return v
    return None


def _set_si_vacio(destino, valor, permitir_sobrescribir=False):
    if not _valor_util(valor):
        return
    actual = st.session_state.get(destino)
    if permitir_sobrescribir or not _valor_util(actual) or str(actual).upper().startswith("SIN_"):
        st.session_state[destino] = valor


def sincronizar_desde_app_principal():
    """Toma identidad policial y actuacion activa cargadas en APP, sin duplicar campos."""
    ni = _buscar_global(["ni", "ni_policial", "pin_ni", "identidad_ni", "sivap_ni", "siv_ni", "usuario_ni"])
    nombre = _buscar_global(["nombre_apellido", "nombre_y_apellido", "nombre_policial", "identidad_nombre", "sivap_nombre", "nombre"])
    jerarquia = _buscar_global(["jerarquia", "rango", "grado", "sivap_jerarquia"])
    dep_pol = _buscar_global(["dependencia_policial", "dependencia", "reparticion", "repartición", "sivap_dependencia"])
    rol = _buscar_global(["rol_operativo", "rol", "sivap_rol"])

    nro_acta = _buscar_global(["numero_acta", "nro_acta", "acta_numero", "acta", "sivap_numero_acta"])
    fecha_acta = _buscar_global(["fecha_actuacion", "actuacion_fecha", "fecha", "sivap_fecha"])
    hora_acta = _buscar_global(["hora_actuacion", "actuacion_hora", "hora", "sivap_hora"])
    lugar = _buscar_global(["lugar_entrevista", "lugar_hecho", "lugar", "sivap_lugar"])
    dep_acta = _buscar_global(["dependencia_acta", "reparticion_dependencia", "repartición_dependencia", "reparticion", "dependencia"])
    personal_act = _buscar_global(["personal_actuante", "funcionario", "personal_policial", "actuante"])

    _set_si_vacio("b3_ni_policial", str(ni).strip() if ni is not None else "")
    _set_si_vacio("b3_nombre_policial", str(nombre).strip() if nombre is not None else "")
    _set_si_vacio("b3_jerarquia", str(jerarquia).strip() if jerarquia is not None else "")
    _set_si_vacio("b3_dependencia_policial", str(dep_pol).strip() if dep_pol is not None else "")
    _set_si_vacio("b3_rol_operativo", str(rol).strip() if rol is not None else "")

    _set_si_vacio("b3_numero_acta", str(nro_acta).strip() if nro_acta is not None else "")
    fecha_norm = _normalizar_fecha(fecha_acta)
    if fecha_norm:
        _set_si_vacio("b3_fecha_entrevista", fecha_norm)
    _set_si_vacio("b3_hora_entrevista", str(hora_acta).strip() if hora_acta is not None else "")
    _set_si_vacio("b3_lugar_entrevista", str(lugar).strip() if lugar is not None else "")
    _set_si_vacio("b3_dependencia_acta", str(dep_acta).strip() if dep_acta is not None else "")

    # Si APP no trae un campo personal_actuante especifico, usamos el nombre policial validado.
    if _valor_util(personal_act):
        _set_si_vacio("b3_personal_actuante", str(personal_act).strip())
    elif _valor_util(st.session_state.get("b3_nombre_policial")):
        _set_si_vacio("b3_personal_actuante", st.session_state.get("b3_nombre_policial"))


def restaurar_respaldo_b3():
    backup = st.session_state.get("_b3_respaldo_vivo")
    if not isinstance(backup, dict):
        return
    for k in B3_DATA_KEYS:
        if k in backup and _valor_util(backup.get(k)) and not _valor_util(st.session_state.get(k)):
            st.session_state[k] = backup.get(k)


def actualizar_respaldo_b3():
    st.session_state["_b3_respaldo_vivo"] = {k: st.session_state.get(k) for k in B3_DATA_KEYS if k in st.session_state}


# Se aplica antes de dibujar widgets.
restaurar_respaldo_b3()
sincronizar_desde_app_principal()
...
def importar_json_bloque3(data):
    ...
    if firma:
        canvas_b64 = (
            firma.get("firma_base64")
            or firma.get("firma_preview_base64")
            or firma.get("firma_canvas_base64")
            or ""
        )
...
def normalizar_firma_imagen(img_rgba, ancho_final=700, alto_final=220):
    """
    Crea una copia interna limpia de la firma: recorta el trazo y lo centra sobre fondo blanco.
    Esto es la imagen que viaja por JSON y se usa en el PDF del actante.
    """
    ...
    for y in range(h):
        for x in range(w):
            r, g, b, a = pix[x, y]
            # Trazo oscuro visible. Evita tomar el fondo blanco.
            if a > 20 and r < 190 and g < 190 and b < 190:
                xs.append(x)
                ys.append(y)

    fondo_final = Image.new("RGB", (ancho_final, alto_final), (255, 255, 255))
    if not xs or not ys:
        return fondo_final
    ...
    recorte = img_rgba.crop((x1, y1, x2, y2))
    recorte_rgb = Image.new("RGB", recorte.size, (255, 255, 255))
    recorte_rgb.paste(recorte, mask=recorte.split()[3])

    max_w, max_h = ancho_final - 80, alto_final - 50
    recorte_rgb.thumbnail((max_w, max_h))
    px = (ancho_final - recorte_rgb.width) // 2
    py = (alto_final - recorte_rgb.height) // 2
    fondo_final.paste(recorte_rgb, (px, py))
    return fondo_final
...
def canvas_a_base64(image_data):
    ...
    try:
        img = Image.fromarray(image_data.astype("uint8"), "RGBA")
        firma_limpia = normalizar_firma_imagen(img)
        buf = io.BytesIO()
        firma_limpia.save(buf, format="PNG")
        return base64.b64encode(buf.getvalue()).decode("utf-8")
    except Exception:
        return ""


def imagen_desde_base64(b64):
...
class PDFB3(FPDF):
    ...
    def footer(self):
        # Documento publico: sin lema, propaganda ni marcas ajenas al acta.
        pass
...
def generar_pdf_completo_b3():
    """
    PDF BLOQUE 3 - Acta de Entrevista prolija.
    Criterios S.I.V.A.P.:
    - Sin Art. 177 CPP / Art. 275 CP.
    - Sin formula de cierre en el acta de entrevista.
    - Datos compactos y legibles.
    - Firma digital incorporada.
    - Hoja separada de Derechos de la Victima.
    """
    ...
    def cell_box(x, y, w, h, label, value, label_w=34):
        pdf.set_xy(x, y)
        pdf.set_font("Arial", "B", 8)
        pdf.cell(label_w, h, limpiar_pdf(label), border=1, align="L")
        pdf.set_font("Arial", "", 8)
        pdf.cell(w - label_w, h, limpiar_pdf(value or ""), border=1, align="L")

    def section_title(title):
        pdf.ln(3)
        pdf.set_fill_color(235, 235, 235)
        pdf.set_font("Arial", "B", 10)
        pdf.cell(0, 7, limpiar_pdf(title), border=0, ln=True, align="L", fill=True)
        pdf.ln(2)

    def label_value_line(label, value):
        pdf.set_font("Arial", "B", 9)
        pdf.cell(39, 6, limpiar_pdf(label), border=0)
        pdf.set_font("Arial", "", 9)
        pdf.multi_cell(0, 6, limpiar_pdf(value or ""), border=0)

    def texto_seguro(v):
        return limpiar_pdf(str(v or "").strip())

    lugar, fecha_txt, hora = datos_apertura_pdf()
    f = filiacion_victima()
    dep = val("b3_dependencia_acta") or val("b3_dependencia_policial") or ""
    personal = val("b3_personal_actuante") or val("b3_nombre_policial")
    jerarquia = val("b3_jerarquia")
    ni = val("b3_ni_policial")
    acta = val("b3_numero_acta")
    correo = f.get("correo", "")
    tel = " / ".join([x for x in [f.get("telefono"), f.get("celular")] if str(x).strip()])

    # =====================================================
    # Hoja 1 - Acta de Entrevista
    # =====================================================
    pdf.add_page()

    # Titulo principal
    pdf.set_font("Arial", "B", 13)
    pdf.cell(0, 8, limpiar_pdf("ACTA DE ENTREVISTA"), ln=True, align="C")
    pdf.ln(2)

    # Cuadro administrativo superior
    y = pdf.get_y()
    x = pdf.l_margin
    total_w = pdf.w - pdf.l_margin - pdf.r_margin
    col_w = total_w / 3
    h = 7
    cell_box(x, y, col_w, h, "Acta Nro.", acta, label_w=23)
    cell_box(x + col_w, y, col_w, h, "Fecha", fecha_txt, label_w=20)
    cell_box(x + col_w * 2, y, col_w, h, "Hora", str(hora), label_w=18)
    pdf.set_y(y + h + 3)

    # Datos de recepcion - cuadro compacto y alineado
    section_title("DATOS DE RECEPCION")
    x = pdf.l_margin
    y = pdf.get_y()
    total_w = pdf.w - pdf.l_margin - pdf.r_margin
    col_w = total_w / 2
    ...
    datos_func = ""
    if jerarquia:
        datos_func += f"{jerarquia}"
    if ni:
        datos_func += (" - " if datos_func else "") + f"NI {ni}"
    cell_box(x, y, col_w, row_h, "Lugar", lugar, label_w=24)
    cell_box(x + col_w, y, col_w, row_h, "Dependencia", dep, label_w=31)
    y += row_h
    cell_box(x, y, col_w, row_h, "Personal policial", personal, label_w=38)
    cell_box(x + col_w, y, col_w, row_h, "Jerarquia / NI", datos_func, label_w=34)
    pdf.set_y(y + row_h + 3)

    # Datos de persona entrevistada en cuadro compacto
    section_title("DATOS DE LA PERSONA ENTREVISTADA")
    x = pdf.l_margin
    y = pdf.get_y()
    total_w = pdf.w - pdf.l_margin - pdf.r_margin
    col_w = total_w / 2
    ...
    cell_box(x, y, col_w, row_h, "Apellido y nombre", nombre, label_w=36)
    cell_box(x + col_w, y, col_w, row_h, "DNI", f.get("dni", ""), label_w=18)
    y += row_h
    cell_box(x, y, col_w, row_h, "Edad", f.get("edad", ""), label_w=20)
    cell_box(x + col_w, y, col_w, row_h, "Fecha nac.", f.get("fecha_nacimiento", ""), label_w=27)
    y += row_h
    cell_box(x, y, col_w, row_h, "Nacionalidad", f.get("nacionalidad", ""), label_w=31)
    cell_box(x + col_w, y, col_w, row_h, "Estado civil", f.get("estado_civil", ""), label_w=29)
    y += row_h
    cell_box(x, y, col_w, row_h, "Profesion/Ocup.", f.get("profesion", ""), label_w=34)
    cell_box(x + col_w, y, col_w, row_h, "Caracter", f.get("condicion", ""), label_w=24)
    y += row_h
    cell_box(x, y, total_w, row_h, "Domicilio", f.get("domicilio", ""), label_w=27)
    y += row_h
    cell_box(x, y, col_w, row_h, "Telefono/Cel.", tel, label_w=33)
    cell_box(x + col_w, y, col_w, row_h, "Correo", correo, label_w=22)
    pdf.set_y(y + row_h + 3)

    # Relato principal
    section_title("RELATO DE LA ENTREVISTA")
    pdf.set_font("Arial", "", 10)
    pdf_multi(pdf, relato_pdf, alto=6)
    pdf.ln(6)

    # Firmas - sin formula de cierre
    if pdf.get_y() > 215:
        pdf.add_page()
    section_title("FIRMAS")
    ...
    right_w = total_w / 2 - 5
    pdf.set_font("Arial", "", 9)
    pdf.cell(left_w, 7, "____________________________", ln=False, align="C")
    pdf.cell(10, 7, "", ln=False)
    pdf.cell(right_w, 7, "____________________________", ln=True, align="C")
    pdf.cell(left_w, 5, limpiar_pdf("Firma persona entrevistada"), ln=False, align="C")
    pdf.cell(10, 5, "", ln=False)
    pdf.cell(right_w, 5, limpiar_pdf("Firma personal policial"), ln=True, align="C")
    pdf.ln(4)
    pdf.set_font("Arial", "", 8)
    if nombre or f.get("dni"):
        pdf.cell(left_w, 5, limpiar_pdf(f"Aclaracion/DNI: {nombre} - {f.get('dni','')}"), ln=False, align="C")
    else:
        pdf.cell(left_w, 5, limpiar_pdf("Aclaracion/DNI:"), ln=False, align="C")
    pdf.cell(10, 5, "", ln=False)
    personal_txt = personal or ""
    if jerarquia:
        personal_txt = f"{jerarquia} {personal_txt}".strip()
    if ni:
        personal_txt += f" - NI {ni}"
    pdf.cell(right_w, 5, limpiar_pdf(personal_txt), ln=True, align="C")
    if dep:
        pdf.cell(0, 5, limpiar_pdf(f"Dependencia: {dep}"), ln=True, align="R")
    ...
    section_title("DERECHOS INFORMADOS")
    pdf.set_font("Arial", "", 9)
    derechos = [
    ...
    for idx, d in enumerate(derechos, start=1):
        pdf_multi(pdf, f"{idx}. {d}", alto=5)
    pdf.ln(5)
    ...
    pdf.cell(10, 7, "", ln=False)
    pdf.cell(right_w, 7, "____________________________", ln=True, align="C")
    pdf.cell(left_w, 5, limpiar_pdf("Firma victima/damnificado"), ln=False, align="C")
    pdf.cell(10, 5, "", ln=False)
    pdf.cell(right_w, 5, limpiar_pdf("Firma personal policial"), ln=True, align="C")
    pdf.ln(4)
    pdf.set_font("Arial", "", 8)
    pdf.cell(left_w, 5, limpiar_pdf(f"Aclaracion/DNI: {nom} - {dni}"), ln=False, align="C")
    pdf.cell(10, 5, "", ln=False)
    pdf.cell(right_w, 5, limpiar_pdf(personal_txt), ln=True, align="C")
...
def armar_json_exportacion():
    ...
    datos_bloque = {
        ...
        "firma": {
            ...
            "firma_base64": val("b3_firma_canvas_b64"),
            "firma_preview_base64": val("b3_firma_canvas_b64"),
...
with st.sidebar:
    st.header("Datos tomados desde APP")
    st.success("Identidad policial / actuación sincronizada")
    st.caption("Estos datos vienen de la pantalla principal de S.I.V.A.P.; no se duplican en BLOQUE 3.")
    st.markdown(f"**NI:** {val('b3_ni_policial') or '-'}")
    st.markdown(f"**Nombre:** {val('b3_nombre_policial') or '-'}")
    st.markdown(f"**Jerarquía:** {val('b3_jerarquia') or '-'}")
    st.markdown(f"**Dependencia:** {val('b3_dependencia_policial') or val('b3_dependencia_acta') or '-'}")
    st.markdown(f"**Rol:** {val('b3_rol_operativo') or '-'}")
    st.divider()
    st.markdown(f"**Acta:** {val('b3_numero_acta') or '-'}")
    st.markdown(f"**Fecha:** {val('b3_fecha_entrevista') or '-'}")
    st.markdown(f"**Hora:** {val('b3_hora_entrevista') or '-'}")
    st.markdown(f"**Lugar:** {val('b3_lugar_entrevista') or '-'}")

with st.expander("⚙️ Datos de recepción / actuación — corregir solo si hace falta", expanded=False):
    st.caption("Use esta sección solo si el dato no vino desde APP o necesita corregirse para el PDF.")
    ...
    with c2:
        ...
        st.text_input("Personal actuante", key="b3_personal_actuante")
    with c3:
        st.text_input("Jerarquía", key="b3_jerarquia")
        st.text_input("NI", key="b3_ni_policial")
        st.text_input("Dependencia policial", key="b3_dependencia_policial")



# =====================================================
# FORMULARIO EN UNA SOLA PÁGINA CON DESPLEGABLES
# =====================================================
# Se abandona la navegación por tabs/radio para evitar pérdida de estado.
# Todas las etapas se renderizan en la misma corrida dentro de expander,
# de modo que los widgets permanecen vivos y no se vacían al cambiar de sección.

with st.expander("📥 0. Importar JSON recibido por WhatsApp", expanded=False):
    if st.session_state.pop("_b3_import_ok", False):
        st.success("Datos incorporados. Revise y corrija el BLOQUE 3 antes de imprimir o exportar.")
    if st.session_state.get("_b3_import_error"):
        st.error("No se pudo incorporar el JSON: " + str(st.session_state.pop("_b3_import_error")))

    archivo = st.file_uploader("Subir JSON de BLOQUE 3", type=["json"], key="b3_import_json")
    if archivo is not None:
        try:
            data = json.load(archivo)
            if not es_json_bloque3(data):
                st.error("Este archivo no corresponde a BLOQUE 3. Debe importarse en su bloque correspondiente.")
            else:
...
with st.expander("👤 1. Identificación de víctima / damnificado", expanded=True):
    c1, c2 = st.columns(2)
    with c1:
...
with st.expander("📝 2. Relato inicial", expanded=False):
    st.text_area("Relato inicial de la víctima/damnificado", key="b3_relato_crudo", height=360)

with st.expander("🧭 3. Análisis S.I.V.A.P. — guía dinámica", expanded=False):
    ...
    if any(s in situaciones for s in ["Robo con arma", "Robo / sustracción", "Hurto"]):
        st.markdown("#### Preguntas clave — Robo / sustracción")
    ...
    if "Amenazas" in situaciones:
        st.markdown("#### Preguntas clave — Amenazas")
    ...
    if "Lesiones" in situaciones:
        st.markdown("#### Preguntas clave — Lesiones")
    ...
    if "Aprehensión / retención vinculada" in situaciones:
        st.markdown("#### Preguntas clave — Persona sindicada / datos objetivos")
    ...
    if "Daño" in situaciones:
        st.markdown("#### Preguntas clave — Daño")
    ...
    if "Violencia familiar / contexto familiar" in situaciones:
        st.markdown("#### Preguntas clave — Contexto familiar / vínculo")
    ...
    if "Cámaras / testigos / evidencia" in situaciones:
        st.markdown("#### Preguntas clave — Cámaras / testigos / evidencia")
    ...
    if "Otro" in situaciones:
        st.markdown("#### Preguntas clave — Otro")
        st.text_area("Detalle adicional para integrar al relato", key="b3_otro_detalle", height=120)

with st.expander("📄 4. Relato final / Noticia criminis", expanded=False):
    c1, c2 = st.columns(2)
    with c1:
...
# =====================================================
# FIRMA DIGITAL DIRECTA - FUERA DE EXPANDERS
# =====================================================
# El canvas no se renderiza bien dentro de tabs/expanders en algunos navegadores.
# Por eso la firma queda como sector directo en la página, mientras las demás etapas
# siguen en desplegables para no perder contenido.
st.markdown("---")
st.subheader("🖊️ FIRMA DIGITAL DE LA PERSONA ENTREVISTADA")
st.caption("Firme dentro del recuadro blanco. El sistema guarda una copia digital interna de la firma para insertarla en el PDF y enviarla dentro del JSON por WhatsApp.")
...
if CANVAS_OK:
    ...
    firma_img = imagen_desde_base64(val("b3_firma_canvas_b64"))
    if firma_img is not None:
        st.success("Firma digital capturada. Esta copia viajará en el JSON y será usada en el PDF.")
        st.image(firma_img, caption="Vista previa interna de la firma digital", width=360)
    else:
        st.warning("Todavía no se detectó firma digital. Firme dentro del recuadro blanco.")
else:
...
with st.expander("📤 6. PDF / JSON", expanded=False):
    st.info("El PDF completo genera Acta de Entrevista y Notificación de Derechos de la Víctima. La firma de FIRMA DIGITAL se inserta en el PDF y viaja en el JSON para WhatsApp.")

    if firma_base64_valida(obtener_firma_base64()):
        st.success("Firma digital cargada. Será incorporada al PDF y al JSON.")
    else:
        st.warning("Todavía no se detectó firma digital. Primero firme en FIRMA DIGITAL.")
...
# Mantiene copia viva de los datos reales del BLOQUE 3 dentro de la sesión.
try:
    actualizar_respaldo_b3()
except Exception:
    pass
